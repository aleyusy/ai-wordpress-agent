...
class AIWP_Admin {
    ...
    public static function render_settings_page() {
        ...
        (function(){
            ...
            el('aiwp-test-api')?.addEventListener('click',function(){var r=el('aiwp-test-result');r.textContent='⏳';var d=new FormData();d.append('action','aiwp_test_api');d.append('nonce',nonce);fetch(ajaxurl,{method:'POST',body:d}).then(function(r){return r.json();}).then(function(r){r.success?r.textContent='✅ OK':r.textContent='❌ '+(r.data?.message||'Error');}).catch(function(){r.textContent='❌ Error';});
            el('aiwp-fetch-models')?.addEventListener('click',function(){var s=el('aiwp-models-status'),i=el('model_input');s.textContent='⏳';var d=new FormData();d.append('action','aiwp_fetch_models');d.append('nonce',nonce);fetch(ajaxurl,{method:'POST',body:d}).then(function(r){return r.json();}).then(function(r){if(r.success&&r.data.models){var l=document.createElement('datalist');l.id='model_list';r.data.models.forEach(function(m){var o=document.createElement('option');o.value=m.id;o.textContent=m.name;l.appendChild(o);});var old=document.getElementById('model_list');if(old)old.remove();i.setAttribute('list','model_list');i.parentNode.insertBefore(l,i.nextSibling);s.textContent='✅ '+r.data.models.length;}else s.textContent='❌';}).catch(function(){s.textContent='❌';});});
        })();
        </script>
        <?php
    }
